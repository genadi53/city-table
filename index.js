...
const main = async () => {
  ...
  const getAllPoints = async () => {
    const pr = await new Promise((resolve, reject) => {
      connection.query(
        "select id, city, region, country, latitude, longtitude, population from wp_my_points_v2;",
  ...
  const getAllCountries = async () => {
    const pr = await new Promise((resolve, reject) => {
      connection.query(
        "select distinct country from wp_my_points_v2;",
        function (error, results, fields) {
          if (error) throw error;
          // console.log(results);
          resolve(results);
  ...
  const getAllCities = async () => {
    const pr = await new Promise((resolve, reject) => {
      connection.query(
        "select distinct city from wp_my_points_v2;",
        function (error, results, fields) {
          if (error) throw error;
          // console.log(results);
          resolve(results);
  ...
  const getCitiesByCountry = async (country) => {
    const pr = await new Promise((resolve, reject) => {
      connection.query(
        {
          sql: "select distinct city from wp_my_points_v2 where country = ?;",
          values: [country],
        },
        function (error, results, fields) {
          if (error) throw error;
  ...
  const getCountryByCity = async (city) => {
    const pr = await new Promise((resolve, reject) => {
      connection.query(
        {
          sql: "select distinct country from wp_my_points_v2 where city = ?;",
          values: [city],
        },
        function (error, results, fields) {
          if (error) throw error;
  ...
  const getWithFilters = async (filters) => {
    ...
    if (filters.x1 && filters.y1 && filters.x2 && filters.y2) {
      if (isOther) {
        query +=
          " and latitude >= ? and latitude <= ? and longtitude >= ? and longtitude <= ?";
  ...
  const getAllCitiesInRect = async (rectCoord) => {
    ...
    let query =
      "select id, city, region, country, latitude, longtitude, population from wp_my_points_v2 where latitude >= ? and latitude <= ? and longtitude >= ? and longtitude <= ?;";
    console.log(query);
    // console.log(values)
    const pr = await new Promise((resolve, reject) => {
      connection.query(
